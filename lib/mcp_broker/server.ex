defmodule McpBroker.Server do
  ...
  def init(_client_info, frame) do
    ...
    case :ets.whereis(:mcp_broker_tools_registered) do
      :undefined ->
        ...
        Logger.info("First client connecting - creating ETS table and scheduling tool registration")
        :ets.new(:mcp_broker_tools_registered, [:set, :public, :named_table])
        Process.send_after(self(), :register_tools, 3000)  # Wait for clients to be ready
        {:ok, frame}
      _table ->
        # Tools already registered, get them from the table
        case :ets.lookup(:mcp_broker_tools_registered, :tools) do
          [{:tools, tools}] ->
            Logger.info("Subsequent client connecting - loading #{length(tools)} tools from cache")
            frame = 
  ...
  def handle_tool_call(tool_name, arguments, frame) do
    case ToolAggregator.call_tool(tool_name, arguments) do
      ...
      {:error, reason} ->
        ...
        error = Error.execution("Tool execution failed", %{tool: tool_name, reason: inspect(reason)})
        {:error, error, frame}
    end
  end
  ...
  defp register_dynamic_tools(frame) do
    case ToolAggregator.aggregate_tools() do
      {:ok, tools} ->
        Logger.info("Registering #{length(tools)} dynamic tools: #{Enum.map(tools, & &1.name) |> Enum.join(", ")}")
